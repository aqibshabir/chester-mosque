...
export async function generateStaticParams() {
  const slugQuery = defineQuery('*[_type == "updatesSubPageType"]{"slug": slug.current}');
...
const subPageQuery = defineQuery(
  '*[_type == "updatesSubPageType" && slug.current == $slug][0]{title, publishedAt, content, readTime, "image": image.asset._ref, _updatedAt}'
);

export default async function UpdateSubPage({ params }: UpdateSubPageProps) {
  const { slug } = await params;
  const update = await sanityFetch<SubPageType>(subPageQuery, { slug });
  ...
  return (
    <article className="max-w-[1250px] md:mx-4  xl:mx-auto md:py-4 mb-20">
      <div className="relative w-full aspect-[2/1] md:rounded-md overflow-hidden mb-8">
        <Image
          src={urlFor(update.image).auto('format').url()}
          alt={update.title}
          fill
      ...
      <div className="mx-4 md:mx-0">
        <div className="flex items-center text-black/60 text-sm md:text-base">
          ...
          <p>{update.readTime} min read</p>
        </div>
        <h2 className="text-4xl md:text-6xl font-bold mb-6">{update.title}</h2>

        <div className="prose prose-lg max-w-none">
          <PortableText value={update.content} components={components} />
        </div>
      </div>
    </article>
  );
