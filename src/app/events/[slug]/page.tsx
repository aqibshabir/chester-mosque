...
const subPageQuery = defineQuery(
  '*[_type == "eventsSubPageType" && slug.current == $slug][0]{title, content, readTime, "image": image.asset._ref, _updatedAt}'
);

export default async function EventsSubPage({ params }: EventsSubPageProps) {
  const { slug } = await params;
  const events = await sanityFetch<SubPageType>(subPageQuery, { slug });

  if (!events) {
    return notFound();
  }

  const date = new Date(events._updatedAt).toLocaleDateString('en-GB', {
    day: '2-digit',
    month: 'long',
    year: 'numeric',
  });
    ...
    <article className="max-w-[1250px] md:mx-4  xl:mx-auto md:py-4 mb-20">
      ...
      <div className="mx-4 md:mx-0">
        <div className="flex items-center text-black/60 text-sm md:text-base">
          ...
          <p>{events.readTime} min read</p>
        </div>
        <h2 className="text-4xl md:text-6xl font-bold mb-6">{events.title}</h2>

        <div className="prose prose-lg max-w-none">
          <PortableText value={events.content} components={components} />
        </div>
      </div>
    </article>
  );
